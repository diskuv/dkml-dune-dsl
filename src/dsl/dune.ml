...
module type SYM = sig
  ...
  *)
  ...
  (** [rule_enabled_if boolean] enables the rule if and only if the [boolean] evaluates
      to the string ["false"]. *)
  ...
  (** {4:Boolean-Language Boolean Language}

      The Boolean language allows the user to define simple Boolean expressions that Dune can
      evaluate.

      After an expression is evaluated, it must be exactly the string true or false to be considered
      as a Boolean. Any other value will be treated as an error.
  ...
  (** All of the booleans must be true; equivalently, all the booleans are
        AND-ed together. *)

  val any : [ `BooleanLanguage ] repr list -> [ `BooleanLanguage ] repr
  (** At least one of the booleans must be true; equivalently, all the
        booleans are OR-ed together. *)

  val eq :
    [ `BooleanLanguage ] repr ->
    [ `BooleanLanguage ] repr ->
    [ `BooleanLanguage ] repr
  (** Equals *)
